...
use crate::session::WorkspaceLock;
...
pub struct ServeState {
    ...
    /// Cross-process exclusive lock held for the lifetime of this serve process.
    pub workspace_lock: Arc<std::sync::Mutex<WorkspaceLock>>,
}
...
impl ServeState {
    ...
    pub fn with_project(
        ...
        let workspace_lock =
            WorkspaceLock::acquire(&litecode_dir).map_err(|e| anyhow::anyhow!("{e}"))?;
        ...
        let sessions = Arc::new(SessionManager::new(
            turn_guard.clone(),
            sessions_db_path.to_string_lossy().to_string(),
        ));
