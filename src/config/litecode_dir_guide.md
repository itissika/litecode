...
优先走设置页，也可以按格式改 JSON。`excludes.json` 里不要写密钥。MCP 的 `env` 可以放该服务器需要的变量；本目录建议 gitignore，不要提交。

MCP / 自定义工具：本文件只存**定义**。进对话还要人类在设置页 **Agents** 里打开对应工具（MCP 为 `mcp_<id>`）。这是审阅开关，Agent 写好定义即可，不要替人打开。
...
**怎么生效。** 空闲时改本文件或设置页保存：进程重读定义（JSON 无效则保持上一份），设置页立刻看到增删。对话进行中不重载（与设置页相同），下一轮开始再读盘。下一轮若该服务器已绑定且定义有变，会停旧进程再拉起；设置页也可手动重启。未绑定的不会启动。
...
**怎么生效。** 空闲时改本文件或设置页保存：进程重读定义（JSON 无效则保持上一份），设置页立刻看到增删。对话进行中不重载（与设置页相同）；下一轮读盘。已绑定的下一轮可用。
