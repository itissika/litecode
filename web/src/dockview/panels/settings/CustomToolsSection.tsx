...
export function CustomToolsSection() {
  ...
  useEffect(() => {
    ...
    if (selectedId) {
      ...
      if (found) {
        ...
        setPropertiesText(JSON.stringify(found.schema?.properties ?? {}, null, 2));
        setRequiredText(JSON.stringify(found.schema?.required ?? [], null, 2));
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    if (!/^[a-z][a-z0-9_]*$/.test(name)) {
      setFormError("Name must match [a-z][a-z0-9_]*");
      return;
    }
    ...
    let required: string[];
    try {
      properties = JSON.parse(propertiesText) as Record<string, unknown>;
      if (properties === null || typeof properties !== "object" || Array.isArray(properties)) {
        throw new Error("properties must be a JSON object");
      }
    } catch (err) {
      setFormError(`Invalid properties JSON: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    try {
      const parsed = JSON.parse(requiredText) as unknown;
      if (!Array.isArray(parsed) || parsed.some((x) => typeof x !== "string")) {
        throw new Error("required must be a JSON array of strings");
      }
      required = parsed as string[];
    } catch (err) {
      setFormError(`Invalid required JSON: ${err instanceof Error ? err.message : String(err)}`);
      return;
    }
    const args = argsText
      .split("\n")
    ...
    const payload: CustomToolDefinition = {
      ...
      args,
      timeout: draft.timeout && draft.timeout > 0 ? draft.timeout : 120,
      schema: {
        type: "object",
        properties,
        required,
    ...
    void (async () => {
      try {
        await saveCustomTool(name, payload);
        setIsNew(false);
        setSelectedId(name);
      } catch {
        // toast already shown
