...
describe("TerminalStatusBar", () => {
  ...
  it("holds a quick job's chip visible for at least 1s (no flash)", () => {
    vi.useFakeTimers();
    render(<TerminalStatusBar sessionId="s1" />);
    ...
    // Job finishes after 50ms — chip must NOT vanish with it.
    ...
    // Hold expires at 1s; the chip then plays its exit animation before unmount.
    act(() => vi.advanceTimersByTime(500));
  ...
  it("holds the chip 1s even after a long-running job ends", () => {
    ...
    // Job runs 10s, then ends — the hold applies the same as for a quick job.
  ...
  it("re-arms the hold when a new job starts during the grace window", () => {
    ...
    // Job B starts at t=300ms (inside A's hold) and ends at t=400ms.
    ...
    // B's hold runs from t=300 → 1300ms: chip still up at t=1200ms…
