...
import { deriveToolStatus, isToolCallLive, processGroupAutoOpen } from "./toolCallStatus";
...
describe("deriveToolStatus", () => {
  it("marks a NON-zero leading exit_code as a warning (the command failed, not the call)", () => {
    expect(deriveToolStatus(result("exit_code: 1\nboom\n"), false, "completed")).toBe(
      "warning",
    );
    expect(deriveToolStatus(result("exit_code: 130\n"), false, "completed")).toBe(
      "warning",
    );
  });

  it("keeps a zero exit_code neutral", () => {
    expect(deriveToolStatus(result("exit_code: 0\nall good\n"), false, "completed")).toBe(
      "ok",
    );
  });
  ...
  it("keeps the existing Error:/Warning: prefixes and lifecycle statuses", () => {
    expect(deriveToolStatus(result("Error: nope"), false, "completed")).toBe("failed");
    expect(deriveToolStatus(result("Warning: careful"), false, "completed")).toBe(
      "warning",
    );
...
describe("isToolCallLive / processGroupAutoOpen", () => {
  ...
  it("auto-opens a process group unless it is closed by a message or a terminal stop", () => {
    expect(processGroupAutoOpen({ followedByMessage: false, hasTerminalStop: false })).toBe(
      true,
    );
    expect(processGroupAutoOpen({ followedByMessage: true, hasTerminalStop: false })).toBe(
      false,
    );
    expect(processGroupAutoOpen({ followedByMessage: false, hasTerminalStop: true })).toBe(
      false,
    );
  });
});
